...
VERSION="1.0.0"
SERVICE_NAME="sm-legal-contract"
DISPLAY_NAME="SM Legal Contract"
DESCRIPTION="合同法务管理：合同起草、审批、电子签章、归档和到期提醒"
ALLOWED_HOSTS=[x.strip() for x in os.getenv("SM_ALLOWED_HOSTS","localhost,127.0.0.1,testserver").split(",") if x.strip()]
ITEMS=[]
...
class Item(BaseModel):
 ...
 owner:str=Field(default="企业应用部",max_length=80)
...
def create(payload:Item): item={"id":str(uuid.uuid4()),**payload.model_dump(),"created_at":datetime.now(UTC).isoformat()}; ITEMS.append(item); return item
@app.get("/api/ops/metrics")
def metrics(): total=int(REQUESTS["total"]); return {"service":SERVICE_NAME,"version":VERSION,"requests_total":total,"errors_total":int(REQUESTS["errors"]),"avg_latency_ms":round(REQUESTS["latency_ms_total"]/total,2) if total else 0}
