...
合同数据属于 **机密 / 绝密** 级，承载企业商业秘密与法定义务，本服务须额外满足以下合规要求：

1. **合同数据保密性**
   - 合同正文、谈判记录、签署意愿、对方主体信息落库一律使用 SM4 字段级加密（`SM_SM4_KEY_HEX`），数据库中不以明文存储。
   - 合同附件走对象存储加密桶，访问链路经 API-Gateway 鉴权 + 内网专线，禁止公网直链。
   - 访问合同数据必须命中 RBAC 的 `contract:read` / `contract:sign` 权限，且全量留痕审计（谁、何时、看/改/导出了哪一份合同）。
2. **《电子签名法》合规**
   - 依据《中华人民共和国电子签名法》，可靠电子签名须满足：签名制作数据归签名人专有、签署时仅由签名人控制、签署后对签名与数据电文的任何改动可被发现。
   - 本服务对接的电子签名须使用合法 CA/认证服务机构（C/A）颁发的数字证书，禁用自签名伪签名。
   - 签署过程保留时间戳（TSA）、哈希摘要与存证证据链，证据链不可篡改、可导出用于司法举证。
   - 合同存储期限不短于合同权利义务终止后 **10 年**，满足诉讼时效与审计追溯要求。
3. **个人信息保护**
   - 合同中涉及的自然人信息按《个人信息保护法》执行最小必要、目的限定、可撤回原则；敏感个人信息单独授权。
